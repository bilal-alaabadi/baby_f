...
const cards = [
  { id: 1, image: card1, title: 'مستلزمات المواليد' },
  { id: 2, image: card2, title: 'الألعاب' },
];

const HeroSection = () => {
  const navigate = useNavigate();

  // ✅ عند الضغط على أي كرت ينتقل مباشرة إلى صفحة /shop فقط
  const handleClick = () => {
    navigate('/shop');
  };
  ...
  return (
    <section className="px-4 py-8">
      <div className="relative text-center" dir="rtl">
        <h2 className="text-[32px] font-normal text-[#F79B6A] mb-1">
      ...
      <div className="grid grid-cols-2 gap-4 md:gap-6">
        {cards.map((card) => (
          <button
            key={card.id}
            onClick={handleClick} // ✅ لم نعد نمرر category
